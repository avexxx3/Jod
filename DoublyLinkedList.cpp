...
struct App {
  ...
  // Uint32 wav_length;
  // Uint8 *wav_buffer;
  // std::unordered_map<int, double> beatMap;

  App() {
    sdl_utils_Init(windowTitle, &window, &renderer, &font);
    linkedList = new CharacterLinkedList(font, renderer, window);
    capsLock = SDL_GetModState() & KMOD_CAPS;
    SDL_GetWindowSize(window, &windowWidth, &windowHeight);

    // SDL_LoadWAV("res/phonk.wav", &wav_spec, &wav_buffer, &wav_length);
  ...
  void shakeScreen() {
    if (swayX == targetX && swayY == targetY) {
      struct timespec ts;
      clock_gettime(CLOCK_MONOTONIC, &ts);
      srand((time_t)ts.tv_nsec);
      targetX = (rand() % 5 * ((rand() % 2) ? -1 : 1));
      targetY = (rand() % 5 * ((rand() % 2) ? -1 : 1));
  ...
  void setScale() {
    ...
    while (ptr->next) {
      if (ptr->next->data->cStr[0] == '\n' || !ptr->next->next) {
        newScale = (1260.0 / 2) / (ptr->data->rect->x + (10 - targetX));
  ...
  void centerCursor() {
    ...
    cursorXVel = (targetCursorX - cursorRect.x) / divisor;
    cursorYVel = (targetCursorY - cursorRect.y) / divisor;
    ...
    viewportXVel = (targetViewportX - viewportRect.x) / divisor;
    viewportYVel = (targetViewportY - viewportRect.y) / divisor;
  ...
  void pasteClipboard() {
    ...
    linkedList->updatePosition(linkedList->head);
  }
